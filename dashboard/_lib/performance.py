...
def render(DATA, navigate_to):
    ...
    with tab1:
        ...
        fig = go.Figure()
        colors_auc = ["#2196F3"] * 5; colors_auc[2] = "#64ffda"
        ...
        st.markdown('<div class="risk-low"><strong style="color:#64ffda">Key Finding</strong><br>'
                    '<span style="color:#ccd6f6">Graph augmentation (temporal k-NN edges) > model complexity.</span></div>', unsafe_allow_html=True)
    ...
    with tab2:
        ...
        fig_r.update_layout(polar=dict(radialaxis=dict(range=[0,1], gridcolor="rgba(255,255,255,0.1)", color="#8892b0"),
                            angularaxis=dict(gridcolor="rgba(255,255,255,0.1)", color="#ccd6f6"), bgcolor="rgba(0,0,0,0)"),
    ...
    with tab3:
        ...
        st.markdown('<div style="background:rgba(100,255,218,0.08); border-radius:12px; padding:20px; text-align:center">'
                    ...
                    '<p style="color:#64ffda">+178% vs GCN</p></div>', unsafe_allow_html=True)
