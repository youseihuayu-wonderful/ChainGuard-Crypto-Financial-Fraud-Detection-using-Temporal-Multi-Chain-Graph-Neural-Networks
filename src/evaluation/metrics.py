...
All metrics work with binary classification:
    - Class 0: illicit (positive class — what we want to detect)
    - Class 1: licit (negative class)
"""
...
def compute_metrics(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.5) -> dict:
    ...
    Args:
        y_true: Ground truth labels (0=illicit, 1=licit).
...
def print_report(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.5):
    ...
    print(f"\n{classification_report(y_true, y_pred, target_names=['illicit', 'licit'])}")
